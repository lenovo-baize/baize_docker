...
function bootIpfsPeer {
    index=$1
    hostName=ipfs_host_share_${index}

    ipfs_staging=/tmp/ipfs_staging_share_${index}
    rm -rf $ipfs_staging
    mkdir -p $ipfs_staging

    ipfs_data=/tmp/ipfs_data_share_${index}
    rm -rf $ipfs_data
    mkdir -p $ipfs_data
...
function rmIpfsHosts {
    dockerContainers=$(docker ps -a | awk '$2~/share/ {print $1}')
